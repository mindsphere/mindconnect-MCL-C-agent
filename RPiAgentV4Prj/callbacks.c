...
extern char *szRegistrationFile;
...
mcl_error_t custom_entercs_function(void)
{
	return MCL_OK;
	//pthread_mutex_lock( &cs_mutex );
}
mcl_error_t custom_leavecs_function(void)
{
	return MCL_OK;
...
mcl_error_t custom_load_function_shared_secret(char **client_id, char **client_secret, char **registration_access_token, char **registration_uri)
{
    ...
    mcl_error_t code = mcl_file_util_fopen(szRegistrationFile, "r", &file_descriptor);
...
{
    void *file_descriptor = NULL;
    mcl_error_t code = mcl_file_util_fopen(szRegistrationFile, "w", &file_descriptor);
